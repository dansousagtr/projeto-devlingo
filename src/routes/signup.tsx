...
function SignupPage() {
  ...
  const onSubmit = async (values: SignUpFormValues) => {
    ...
    if (!result.success) {
      setSubmitError(result.errorMessage ?? 'Erro ao criar conta.');
      return;
    }
  ...
  const hasError = (field: keyof SignUpFormValues) => Boolean(errors[field]);
          ...
          <form className="mt-10 space-y-6" onSubmit={handleSubmit(onSubmit)} noValidate>
            <div className="space-y-3">
              ...
              <input
                ...
                className={`w-full rounded-[1.25rem] border px-4 py-3 text-sm text-slate-900 transition duration-200 ease-out placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-white disabled:cursor-not-allowed disabled:bg-slate-100 ${
                  errors.name
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                autoComplete="new-password"
                placeholder="••••••••"
                aria-invalid={hasError('confirmPassword') ? 'true' : 'false'}
